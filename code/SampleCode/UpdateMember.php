...
<html>
	...
	<body>
		<p><Strong>MailChimp member update</Strong></p>
		<p>Please provide the member details</p>
		<form method="POST" enctype="application/x-www-form-urlencoded">
			<table>
				<tbody>
					...
					<tr>
						<td>Member ID</td>
						<td><input type="text" name="member_id" value="e788f6f979a66ea2e8b6451888511cbf"></td>
					</tr>
					<tr>
						<td>Email Address</td>
						<td><input type="text" name="email_address" value="anu.nayyar@test.com"></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td><input type="submit" value="Update Existing Member"></td>
					</tr>
				</tbody>
			</table>
		</form>
...
<?php
	...
	if (!empty($_POST)) {
		...
		if($mailChimpValidate) {
			...
			$transaction->setEmailAddress($_POST['email_address'])
						->setStatus('unsubscribed');

			//Create Object of MailChimp.
			$mailChimpObj = new MailChimp();
			//Call update Member Function.
			$mailChimpMemberID = $mailChimpObj->updateExistingMember($transaction,$_POST['id'],$_POST['member_id']);

			if($mailChimpMemberID) {
				echo "<div class='msgbox success'>Member has been successfully updated in the list with ID: ".$mailChimpMemberID."</div>";
			}
